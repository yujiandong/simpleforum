...
if(!$isGuest) {
    ...
    if ($me->isActive()) {
        $topicOp['follow'] = Favorite::checkFollow($me->id, Favorite::TYPE_TOPIC, $topic['id'])?Html::a('<i class="fa fa-star fa-lg"></i>', ['service/unfavorite', 'type'=>'topic', 'id'=>$topic['id']], [
            'title' => '取消收藏',
            'data' => [
                'method' => 'post',
            ]]):Html::a('<i class="fa fa-star-o fa-lg"></i>', ['service/favorite', 'type'=>'topic', 'id'=>$topic['id']], ['title' => '加入收藏']);
    }
    $topicOp['sms'] = Html::a('<i class="fa fa-envelope fa-lg"></i>', ['service/sms', 'to'=>Html::encode($topic['author']['username'])], ['title' => '私信Ta']);
...
<div class="panel panel-default sf-box">
    <div class="panel-heading">
        ...
        <small class="gray">
        <?php
            echo '<strong><i class="fa fa-user"></i>', Html::a(Html::encode($topic['author']['username']), ['user/view', 'username'=>Html::encode($topic['author']['username'])]), Util::getGroupRank($topic['author']['score']),'</strong>',
...
/*      if ($indexPage > 0) {
        ...
        echo Html::a('<i class="fa fa-envelope fa-lg"></i>', ['service/sms', 'to'=>Html::encode($comment['author']['username'])], ['title' => '私信Ta']), ' | ';
    }
                    echo $comment['position'].'楼',
                '</small>
                <small class="gray"><strong>', Html::a('<i class="fa fa-user"></i>'.Html::encode($comment['author']['username']), ['user/view', 'username'=>Html::encode($comment['author']['username'])]), Util::getGroupRank($comment['author']['score']), '</strong> ',$comment['author']['comment'],' •  ',
